        ...
        <div class="row justify-content-between">
            <div class="col-12">
                <div class="d-flex justify-content-end">
                    <a href="javascript:;" class="btn btn-primary m-b-xs">
                                        ...
                                        @foreach($group22 as $ids => $ss)
                                            <tr>
                                                ...
                                                @if($ss != 'Peralatan Komunikasi')
                                                    <td><input type="number" name="jumlah[{{$ids}}]" min="0" class="input-custom"></td>
                                                    <td class="text-center"><input type="radio" name="layak[{{$ids}}]" id="layak{{$ids}}"></td>
                                                    <td class="text-center"><input type="radio" name="layak[{{$ids}}]" id="tidak_layak{{$ids}}"></td>
                                                @else
                                                    @for($i=1;$i<=3;$i++)
                                                        <td class="bg-header"></td>
                                                    @endfor
                                            ...
                                            @php $no = 1; @endphp
                                                @foreach($s as $key => $data)
                                                    <tr>
                                                        ...
                                                        <td>{{$data}}</td>
                                                        <td><input type="number" name="jumlah[{{$key}}]" min="0" class="input-custom"></td>
                                                        <td class="text-center"><input type="radio" name="layak[{{$key}}]" id="layak{{$key}}"></td>
                                                        <td class="text-center"><input type="radio" name="layak[{{$key}}]" id="tidak_layak{{$key}}"></td>
                                                    </tr>
                                                    @php $no++ @endphp
                                                @endforeach
                                            @php $grp1++; @endphp
